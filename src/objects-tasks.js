...
function removeProperties(obj, keys) {
  const resObj = { ...obj };
  keys.forEach((key) => {
    if (resObj[key]) delete resObj[key];
  });
...
function makeWord(lettersObject) {
  ...
  const wordLength = values.flat().length;
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === 25) sellerMoney += 25;
    if (queue[i] > 25) {
      if (queue[i] - 25 > sellerMoney + 25) return false;
      sellerMoney += 25;
    }
  }
  return true;
...
const cssSelectorBuilder = {
  ...
  element(value) {
    return this.createNewElem(value, 1, 'hasElem');
  },

  id(value) {
    return this.createNewElem(`#${value}`, 2, 'hasID');
  },

  class(value) {
    return this.createNewElem(`.${value}`, 3);
  },

  attr(value) {
    return this.createNewElem(`[${value}]`, 4);
  },

  pseudoClass(value) {
    return this.createNewElem(`:${value}`, 5);
  },

  pseudoElement(value) {
    return this.createNewElem(`::${value}`, 6, 'hasPseudoElem');
  },

  combine(selector1, combinator, selector2) {
    const res = `${selector1.result} ${combinator} ${selector2.result}`;
    const o = Object.create(this, { result: { value: res } });
    return o;
  },

  createNewElem(value, order, opt = null) {
    if (
      (opt === 'hasElem' && this.hasElem) ||
      (opt === 'hasID' && this.hasID) ||
      (opt === 'hasPseudoElem' && this.hasPseudoElem)
    ) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    if (order < this.lastElemOrder) {
      throw new Error(
